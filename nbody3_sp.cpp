...
void help()
{
  fprint(stderr,"nbody3 --help|-h --nparticles|-n --nsteps|-s --stepsize|-t\n");
}
...
# define ACC_FUNC accel_opt2
  ...
  if (1 && n == 2)
  {
    /* Initialize a 2-body problem with large ratio and tangential
     * velocity fir the small body. */

     pos_array(0,0) = 0.0; pos_array(0,1) = 0.0; pos_array(0,2) = 0.0;
     vel_array(0,0) = 0.0, pos_array(0,1) = 0.0; pos_array(0,2) = 0.0;
     mass[1] = 1.0;
  }
  else
  {
  ...
  {
    /* 1. Compute the acceleration each object. */
  ...
  //printf("Average time = %f (ms) per step with %d elements %.sf KB over %d steps %.3f%%, %.3f%%, %.3f%%\n", t_calc*1000.0/num_steps, n. nkbytes, num_steps, 100*t_accel/t_calc, 100*t_update/t_calc, 100*t_search/t_calc);
  printf("Average time = %f (ms) per step with %d elements %.2f KB over %d steps %f %f %f\n", t_calc*1000.0/num_steps, n, nkbytes, num_steps, t_accel*1000/num_steps, t_update*1000/num_steps, t_searc*1000/num_steps);
